...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .t-val.meeting    { color: var(--blue);   text-shadow: 0 0 6px var(--blue); }
    ...
    .expr-worried  #tama-svg { filter: drop-shadow(0 0 16px #f9731666); }
    .expr-alarmed  #tama-svg { filter: drop-shadow(0 0 22px #ef444488); }
    .expr-meeting  #tama-svg { filter: drop-shadow(0 0 16px #44aaff66); }
    .expr-excited  #tama-svg { filter: drop-shadow(0 0 22px #cc66ff88); }
    ...
    .expr-meeting  .tama-ring { stroke: #44aaff; }
    .expr-excited  .tama-ring { stroke: #cc66ff; }

    .bot-label { display: flex; align-items: center; gap: 4px; margin-top: 7px; }
    #tama-mood {
      font-size: 10px;
      font-weight: bold;
      letter-spacing: 2px;
    ...
    .expr-happy    #tama-mood { color: #22c55e; text-shadow: 0 0 8px #22c55e; }
    .expr-thinking #tama-mood { color: #eab308; text-shadow: 0 0 8px #eab308; }
    .expr-worried  #tama-mood { color: #f97316; text-shadow: 0 0 6px #f97316; }
    .expr-alarmed  #tama-mood { color: #ff4444; text-shadow: 0 0 9px #ff4444; }
    .expr-meeting  #tama-mood { color: #44aaff; text-shadow: 0 0 8px #44aaff; }
    .expr-excited  #tama-mood { color: #cc66ff; text-shadow: 0 0 9px #cc66ff; }
    ...
    @keyframes shake      { 0%,100%{transform:translateX(0) rotate(0deg)} 15%{transform:translateX(-6px) rotate(-3deg)} 30%{transform:translateX(6px) rotate(3deg)} 45%{transform:translateX(-5px) rotate(-2deg)} 60%{transform:translateX(5px) rotate(2deg)} 80%{transform:translateX(-2px) rotate(-1deg)} }
    ...
    #tama-body-group { animation: bob 2.8s ease-in-out infinite; transform-origin: 100px 108px; }
    ...
    .eye-blink     { transform-box: fill-box; transform-origin: center; animation: blink 3.8s ease-in-out infinite; }
  ...
  <div class="top-panel">
    ...
    <div class="status-terminal">
      <div class="t-line">
        ...
        <span id="now-mode" class="t-val">FOCUS</span>
      </div>
      <div class="t-line">
        <span class="t-key">CONTEXT</span>
        <span class="t-sep">►</span>
        <span id="now-context" class="t-val dim">—</span>
      </div>
      <div class="t-line">
        <span class="t-key">AGENT  </span>
        <span class="t-sep">►</span>
        <span id="proc-status" class="t-val idle">IDLE</span>
      </div>
      <div class="t-line">
        <span class="t-key">NUDGES </span>
        <span class="t-sep">►</span>
        <span id="nudge-count" class="t-val ok">0 ACTIVE</span>
      </div>
      <div class="t-line">
        <span class="t-key">LOG    </span>
        <span class="t-sep">►</span>
        <span id="proc-log" class="t-val dim t-log"></span>
      </div>
    </div>

    <div class="mini-bot-area">
        ...
        <svg id="tama-svg" viewBox="0 0 200 210" xmlns="http://www.w3.org/2000/svg">
          ...
          <g id="tama-body-group">
            ...
            <g class="mouth-thinking">
              <path d="M 83 130 Q 100 127 117 130" stroke="#d0d0d0" stroke-width="2.5" fill="none" stroke-linecap="round"/>
      ...
      <div class="bot-label">
        <span id="tama-mood">HAPPY</span>
        <span class="cursor">█</span>
      </div>
      <div id="tama-msg">All caught up!</div>
    </div>
...
<script>
  ...
  const EXPR_DATA = {
    ...
    worried:  { mood: "WORRIED",  msg: "Things need attention" },
    alarmed:  { mood: "ALERT",    msg: "Urgent — act now!" },
  ...
  function render(state) {
    ...
    if (prevProcessing && !isProcessing && expr === "happy") {
      ...
      excitedTimer = setTimeout(() => { currentExpr = ""; setExpression("happy"); }, 2800);
    ...
    const modeEl = document.getElementById("now-mode");
    modeEl.textContent = mode.toUpperCase();
    modeEl.className = "t-val " + (mode === "meeting" ? "meeting" : "ok");
    ...
    procEl.textContent = isProc ? "RUNNING" : "IDLE";
    procEl.className   = "t-val " + (isProc ? "processing" : "idle");
    document.getElementById("proc-log").textContent = state.processingLog || "";

    // Nudge count
    const nudges = (state.nudges || []).filter(n => !n.acked);
    const ncEl = document.getElementById("nudge-count");
    ncEl.textContent = nudges.length + " ACTIVE";
    ncEl.className = "t-val " + (nudges.length > 0 ? (nudges.some(n => n.priority === "high") ? "alarmed" : "worried") : "ok");
    ...
    const checkFor = (status, pri) => {
      if (status === "done")        return \`<span class="tchk done">[x]</span>\`;
      if (status === "in_progress") return \`<span class="tchk prog">[~]</span>\`;
      return \`<span class="tchk \${esc(pri)}">[ ]</span>\`;
    };
  ...
  async function fetchState() {
    try {
      const r = await fetch("/api/alfred/state");
  ...
  async function triggerPoll() {
    ...
    try { await fetch("/api/adapters/granola/actions/poll-now", { method:"POST" }); } catch(e) {}
    setTimeout(() => { btn.disabled = false; btn.textContent = "POLL"; fetchState(); }, 3000);
  }

  // ── SSE ──────────────────────────────────────────────────────
  // State is embedded directly in alfred.state.updated — no second fetch needed
  const FETCH_ON = new Set(["alfred.processing.done","alfred.processing.started"]);
  ...
  function connectSSE() {
    ...
    es.onmessage = (e) => {
      try {
        ...
        if (d.channel === "alfred.state.updated" && d.state) {
          render(d.state); // render directly — zero extra round-trip
        } else if (FETCH_ON.has(d.channel)) {
          fetchState();   // fallback fetch for non-state events
        }
      } catch(e) {}
    };
  }
  ...
  fetchState();
  connectSSE();
  setInterval(fetchState, 60000); // 60s fallback poll only
</script>
</body>
</html>`;
}
